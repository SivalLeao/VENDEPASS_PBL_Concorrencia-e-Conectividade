...
func manipularConexao(cliente net.Conn, id *int, rotas map[string]int, cliente_rotas map[int][]string) {
	...
	*id++
	...
	for {
		...
		if len(comando) < 2 {                    //Se a mensagem não for particionada corretamente, exibe uma mensagem de erro
			...
			fmt.Println("Desconectando cliente")
			return
		}
		...
		case "3": //Sair
			...
			fmt.Println("Cliente:", rotas["Fortaleza"], "Comprou fortaleza")
			return
...
func main() {
	...
	*id = 1                //ID é inicializado com valor 1. Ou seja, não há nenhum cliente ID 0
	...
	for {
		...
		go manipularConexao(conexao, id, rotas, cliente_rotas) //Manipula a conexão em uma nova thread
	}
}
