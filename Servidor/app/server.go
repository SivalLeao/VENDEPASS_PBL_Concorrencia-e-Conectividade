...
func cabecalho() {
	...
	tamanho := len(endereco_completo)
	espacamento := ""
	if tamanho < 20 {
		espacamento = strings.Repeat(" ", 20-tamanho)
	}

	fmt.Println("=-=-=-=-=-=-==-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-")
	fmt.Println("|  Servidor funcionando no endereço:\033[32m", endereco+":"+porta+espacamento+"\033[0m|")
	fmt.Print("=-=-=-=-=-=-==-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n\n")
}
